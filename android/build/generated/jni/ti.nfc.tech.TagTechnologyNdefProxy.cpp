...
Handle<FunctionTemplate> TagTechnologyNdefProxy::getProxyTemplate()
{
	...
	javaClass = titanium::JNIUtil::findClass("ti/nfc/tech/TagTechnologyNdefProxy");
	HandleScope scope;
...
Handle<Value> TagTechnologyNdefProxy::writeNdefMessage(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TagTechnologyNdefProxy::javaClass, "writeNdefMessage", "(Lti/nfc/NdefMessageProxy;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'writeNdefMessage' with signature '(Lti/nfc/NdefMessageProxy;)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> TagTechnologyNdefProxy::getCachedNdefMessage(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TagTechnologyNdefProxy::javaClass, "getCachedNdefMessage", "()Lti/nfc/NdefMessageProxy;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getCachedNdefMessage' with signature '()Lti/nfc/NdefMessageProxy;'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> TagTechnologyNdefProxy::getNdefMessage(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TagTechnologyNdefProxy::javaClass, "getNdefMessage", "()Lti/nfc/NdefMessageProxy;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getNdefMessage' with signature '()Lti/nfc/NdefMessageProxy;'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
