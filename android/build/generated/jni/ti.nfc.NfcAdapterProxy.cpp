...
Handle<FunctionTemplate> NfcAdapterProxy::getProxyTemplate()
{
	...
	javaClass = titanium::JNIUtil::findClass("ti/nfc/NfcAdapterProxy");
	HandleScope scope;
...
Handle<Value> NfcAdapterProxy::enableForegroundDispatch(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(NfcAdapterProxy::javaClass, "enableForegroundDispatch", "(Lti/nfc/NfcForegroundDispatchFilter;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'enableForegroundDispatch' with signature '(Lti/nfc/NfcForegroundDispatchFilter;)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> NfcAdapterProxy::setNdefPushMessage(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(NfcAdapterProxy::javaClass, "setNdefPushMessage", "(Lti/nfc/NdefMessageProxy;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'setNdefPushMessage' with signature '(Lti/nfc/NdefMessageProxy;)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> NfcAdapterProxy::enableForegroundNdefPush(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(NfcAdapterProxy::javaClass, "enableForegroundNdefPush", "(Lti/nfc/NdefMessageProxy;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'enableForegroundNdefPush' with signature '(Lti/nfc/NdefMessageProxy;)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
