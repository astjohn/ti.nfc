...
Handle<FunctionTemplate> TagTechnologyNdefFormatableProxy::getProxyTemplate()
{
	...
	javaClass = titanium::JNIUtil::findClass("ti/nfc/tech/TagTechnologyNdefFormatableProxy");
	HandleScope scope;
...
Handle<Value> TagTechnologyNdefFormatableProxy::format(const Arguments& args)
{
	...
	if (!methodID) {
		methodID = env->GetMethodID(TagTechnologyNdefFormatableProxy::javaClass, "format", "(Lti/nfc/NdefMessageProxy;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'format' with signature '(Lti/nfc/NdefMessageProxy;)V'";
			LOGE(TAG, error);
...
Handle<Value> TagTechnologyNdefFormatableProxy::formatReadOnly(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TagTechnologyNdefFormatableProxy::javaClass, "formatReadOnly", "(Lti/nfc/NdefMessageProxy;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'formatReadOnly' with signature '(Lti/nfc/NdefMessageProxy;)V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
